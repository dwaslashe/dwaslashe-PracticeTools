...
public class PunishmentCommand extends Command implements Listener {
    ...
    private void openGui(int guiID, Player player, OfflinePlayer offlinePlayer) {
        ...
        if (guiID == 0) {
            ...
            ItemStack glass_gray = inventoryHelper.prepareItemStack(Material.STAINED_GLASS_PANE, itemStack -> {
                itemStack.setDurability((short) 7);
                inventoryHelper.editMetaForItemStack(itemStack, itemMeta -> {
                    itemMeta.setDisplayName(" ");
                });
            });
            ItemStack glass_lime = inventoryHelper.prepareItemStack(Material.STAINED_GLASS_PANE, itemStack -> {
                itemStack.setDurability((short) 5);
                inventoryHelper.editMetaForItemStack(itemStack, itemMeta -> {
                    itemMeta.setDisplayName(" ");
                });
            });
            ItemStack glass_red = inventoryHelper.prepareItemStack(Material.STAINED_GLASS_PANE, itemStack -> {
            ...
            ItemStack ban = inventoryHelper.prepareItemStack(Material.SKULL_ITEM, itemStack -> {
                itemStack.setDurability((short) 3);
            ...
            ItemStack kick = inventoryHelper.prepareItemStack(Material.SKULL_ITEM, itemStack -> {
                itemStack.setDurability((short) 3);
                inventoryHelper.editSkullMetaWithProperty(itemStack, "eyJ0ZXh0dXJlcyI6eyJTS0lOIjp7InVybCI6Imh0dHA6Ly90ZXh0dXJlcy5taW5lY3JhZnQubmV0L3RleHR1cmUvNTZiMWQ0OTQ3NzYzOTE3ODE0YWUxNjMyYjgyMDY5NjA5ODkyNzg5NWFhYWYxMjRjZDI5ZWIzNTg1NmFhYTViOSJ9fX0=");
                inventoryHelper.editMetaForItemStack(itemStack, itemMeta -> {
            ...
            ItemStack mute = inventoryHelper.prepareItemStack(Material.SKULL_ITEM, itemStack -> {
                itemStack.setDurability((short) 3);
                inventoryHelper.editSkullMetaWithProperty(itemStack, "eyJ0ZXh0dXJlcyI6eyJTS0lOIjp7InVybCI6Imh0dHA6Ly90ZXh0dXJlcy5taW5lY3JhZnQubmV0L3RleHR1cmUvNTZiMWQ0OTQ3NzYzOTE3ODE0YWUxNjMyYjgyMDY5NjA5ODkyNzg5NWFhYWYxMjRjZDI5ZWIzNTg1NmFhYTViOSJ9fX0=");
                inventoryHelper.editMetaForItemStack(itemStack, itemMeta -> {
        ...
        //1 bany, powod
            ...
            banCache.put(11, "Obraza serwera");
            banCache.put(12, "Wykorzystywanie bledow serwera");
            banCache.put(13, "Reklama");
            banCache.put(14, "Groźby");
            banCache.put(15, "Makro");
            banCache.put(16, "Utrudnianie gry");
            banCache.put(19, "Omijanie kary");
            banCache.put(20, "Życzenie śmierci");
        ...
        //2 mute, powod
            ...
            muteCache.put(10, "Wyzwiska");
            muteCache.put(11, "Przeklinanie");
            muteCache.put(12, "Flood/Spam");
            muteCache.put(13, "Rasizm");
            muteCache.put(14, "Reklama gildi");
            muteCache.put(15, "Wprowadzenie w błąd");
            muteCache.put(16, "Reklama");
    ...
    public void banGui(String reason, Player player, OfflinePlayer offlinePlayer){
        ...
        ItemStack glass_black = inventoryHelper.prepareItemStack(Material.SKULL_ITEM, itemStack -> {
            itemStack.setDurability((short) 15);
            inventoryHelper.editMetaForItemStack(itemStack, itemMeta -> {
                itemMeta.setDisplayName(" ");
            });
        });
        ItemStack glass_gray = inventoryHelper.prepareItemStack(Material.SKULL_ITEM, itemStack -> {
            itemStack.setDurability((short) 7);
            inventoryHelper.editMetaForItemStack(itemStack, itemMeta -> {
                itemMeta.setDisplayName(" ");
            });
        ...
        inventoryHelper.click(e -> {
            ...
            if (e.getSlot() == 10) {
                player.getOpenInventory().close();
                player.chat("/tempipban " + offlinePlayer.getName() + " " + reason + " " + "10m");
                ...
                player.getOpenInventory().close();
                player.chat("/tempipban " + offlinePlayer.getName() + " " + reason + " " + "15m");
                player.sendTitle(Api.fixColor(Main.pluginConfig.getMessages().getIp()), Api.fixColor("&8>> &aPomyślnie wyrzuciłes gracza &e" + offlinePlayer.getName() + " &aza &e" + reason + " &8<<"));
            } else if (e.getSlot() == 12) {
                player.getOpenInventory().close();
                player.chat("/tempipban " + offlinePlayer.getName() + " " + reason + " " + "30m");
                player.sendTitle(Api.fixColor(Main.pluginConfig.getMessages().getIp()), Api.fixColor("&8>> &aPomyślnie wyrzuciłes gracza &e" + offlinePlayer.getName() + " &aza &e" + reason + " &8<<"));
            } else if (e.getSlot() == 13) {
                player.getOpenInventory().close();
                player.chat("/tempipban " + offlinePlayer.getName() + " " + reason + " " + "1h");
                player.sendTitle(Api.fixColor(Main.pluginConfig.getMessages().getIp()), Api.fixColor("&8>> &aPomyślnie wyrzuciłes gracza &e" + offlinePlayer.getName() + " &aza &e" + reason + " &8<<"));
            } else if (e.getSlot() == 14) {
                player.getOpenInventory().close();
                player.chat("/tempipban " + offlinePlayer.getName() + " " + reason + " " + "2h");
                player.sendTitle(Api.fixColor(Main.pluginConfig.getMessages().getIp()), Api.fixColor("&8>> &aPomyślnie wyrzuciłes gracza &e" + offlinePlayer.getName() + " &aza &e" + reason + " &8<<"));
            } else if (e.getSlot() == 15) {
                player.getOpenInventory().close();
                player.chat("/tempipban " + offlinePlayer.getName() + " " + reason + " " + "6h");
                player.sendTitle(Api.fixColor(Main.pluginConfig.getMessages().getIp()), Api.fixColor("&8>> &aPomyślnie wyrzuciłes gracza &e" + offlinePlayer.getName() + " &aza &e" + reason + " &8<<"));
            } else if (e.getSlot() == 16) {
                player.getOpenInventory().close();
                player.chat("/tempipban " + offlinePlayer.getName() + " " + reason + " " + "1d");
                player.sendTitle(Api.fixColor(Main.pluginConfig.getMessages().getIp()), Api.fixColor("&8>> &aPomyślnie wyrzuciłes gracza &e" + offlinePlayer.getName() + " &aza &e" + reason + " &8<<"));
            } else if (e.getSlot() == 20) {
                player.getOpenInventory().close();
                player.chat("/tempipban " + offlinePlayer.getName() + " " + reason + " " + "2d");
                player.sendTitle(Api.fixColor(Main.pluginConfig.getMessages().getIp()), Api.fixColor("&8>> &aPomyślnie wyrzuciłes gracza &e" + offlinePlayer.getName() + " &aza &e" + reason + " &8<<"));
            } else if (e.getSlot() == 21) {
                player.getOpenInventory().close();
                player.chat("/tempipban " + offlinePlayer.getName() + " " + reason + " " + "7d");
                player.sendTitle(Api.fixColor(Main.pluginConfig.getMessages().getIp()), Api.fixColor("&8>> &aPomyślnie wyrzuciłes gracza &e" + offlinePlayer.getName() + " &aza &e" + reason + " &8<<"));
            } else if (e.getSlot() == 22) {
                player.getOpenInventory().close();
                player.chat("/tempipban " + offlinePlayer.getName() + " " + reason + " " + "14d");
                player.sendTitle(Api.fixColor(Main.pluginConfig.getMessages().getIp()), Api.fixColor("&8>> &aPomyślnie wyrzuciłes gracza &e" + offlinePlayer.getName() + " &aza &e" + reason + " &8<<"));
            } else if (e.getSlot() == 23) {
                player.getOpenInventory().close();
                player.chat("/tempipban " + offlinePlayer.getName() + " " + reason + " " + "21d");
                player.sendTitle(Api.fixColor(Main.pluginConfig.getMessages().getIp()), Api.fixColor("&8>> &aPomyślnie wyrzuciłes gracza &e" + offlinePlayer.getName() + " &aza &e" + reason + " &8<<"));
            } else if (e.getSlot() == 24) {
                player.getOpenInventory().close();
                player.chat("/tempipban " + offlinePlayer.getName() + " " + reason + " " + "28d");
                player.sendTitle(Api.fixColor(Main.pluginConfig.getMessages().getIp()), Api.fixColor("&8>> &aPomyślnie wyrzuciłes gracza &e" + offlinePlayer.getName() + " &aza &e" + reason + " &8<<"));
            }
        });
    ...
    public void custommuteGui(String reason, Player player, OfflinePlayer offlinePlayer){
        ...
        ItemStack black_glass = inventoryHelper.prepareItemStack(Material.STAINED_GLASS_PANE, itemStack -> {
            itemStack.setDurability((short) 15);
            inventoryHelper.editMetaForItemStack(itemStack, itemMeta -> itemMeta.setDisplayName(" "));
        });

        ItemStack orange_glass = inventoryHelper.prepareItemStack(Material.STAINED_GLASS_PANE, itemStack -> {
            itemStack.setDurability((short) 1);
            inventoryHelper.editMetaForItemStack(itemStack, itemMeta -> itemMeta.setDisplayName(" "));
        });

        ItemStack arrow_up = inventoryHelper.prepareItemStack(Material.STAINED_GLASS_PANE, itemStack -> {
            itemStack.setDurability((short) 3);
        ...
        ItemStack arrow_down = inventoryHelper.prepareItemStack(Material.SKULL_ITEM, itemStack -> {
            itemStack.setDurability((short) 3);
